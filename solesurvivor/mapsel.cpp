    ...
    */
    ...
    while (frame < Get_Animation_Frame_Count(anim)) {
        ...
        if (factor > 1) {
            ...
            case 33:
                TextPrintBuffer->Fill_Rect(
                    0, 2 * 10, 2 * String_Pixel_Width(GetMapSelString(TXT_NONE)), 2 * (10 + 12), BLACK);
                break;

            case 34:
                TextPrintBuffer->Fill_Rect(
                    0, 2 * 10, 2 * String_Pixel_Width(GetMapSelString(TXT_NONE)), 2 * (10 + 12), TBLACK);
                Alloc_Object(new MultiStagePrintClass(GetMapSelString(TXT_NONE), 0, 10, _othergreenpal));
                break;
