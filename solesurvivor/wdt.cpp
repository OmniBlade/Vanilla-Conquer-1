...
StructType GDI_Defense_Buildings[] = { 
	STRUCT_GTOWER,
	STRUCT_ATOWER
};

StructType Nod_Defense_Buildings[] = {
	STRUCT_TURRET,
	STRUCT_OBELISK
};
...
void CommStats_Set_Current_Object(COORDINATE coord, COORDINATE server_coord, MissionType mission, MissionType suspended_mission, MissionType mission_queue, MissionType server_mission, TARGET navcom, TARGET tarcom)
{
...
void CommStats_Set_Transmission_Stats(int rec_bytes_sec, int sent_bytes_sec, int sent_tcp, int sent_udp, int recieved_tcp, int recieved_udp)
{
...
void CommStats_Mono_Debug_Print(bool fresh)
{
	...
	if ( !(Frame & 15) ) {
		Mono_Set_Cursor(41, 1); Mono_Printf("%03x", (CommStats_Coord & 0x0000FF00) >> 8);
		Mono_Set_Cursor(44, 1); Mono_Printf("%03x", (CommStats_Coord & 0x000000FF)      );
		Mono_Set_Cursor(49, 1); Mono_Printf("%03x", (CommStats_Coord & 0xFF000000) >> 24);
		Mono_Set_Cursor(52, 1); Mono_Printf("%03x", (CommStats_Coord & 0x00FF0000) >> 16);
		Mono_Set_Cursor(41, 2); Mono_Printf("%03x", (CommStats_Server_Coord & 0x0000FF00) >> 8);
		Mono_Set_Cursor(44, 2); Mono_Printf("%03x", (CommStats_Server_Coord & 0x000000FF)      );
		Mono_Set_Cursor(49, 2); Mono_Printf("%03x", (CommStats_Server_Coord & 0xFF000000) >> 24);
		Mono_Set_Cursor(52, 2); Mono_Printf("%03x", (CommStats_Server_Coord & 0x00FF0000) >> 16);
		Mono_Set_Cursor(40, 4); Mono_Printf("%-20s", MissionClass::Mission_Name(CommStats_Mission));
		Mono_Set_Cursor(40, 5); Mono_Printf("%-20s", MissionClass::Mission_Name(CommStats_Suspended_Mission));
		Mono_Set_Cursor(40, 6); Mono_Printf("%-20s", MissionClass::Mission_Name(CommStats_Mission_Queue));
		Mono_Set_Cursor(40, 7); Mono_Printf("%-20s", MissionClass::Mission_Name(CommStats_Server_Mission));
		Mono_Set_Cursor(40, 8); Mono_Printf("%04x", CommStats_NavCom);
		Mono_Set_Cursor(40, 9); Mono_Printf("%04x", CommStats_TarCom);
		Mono_Set_Cursor(42, 12); Mono_Printf("%03d", CommStats_Server_FPS);
		Mono_Set_Cursor(42, 13); Mono_Printf("%03d", CommStats_Client_FPS);
		Mono_Set_Cursor(42, 14); Mono_Printf("%04x", SpeedScale);
		Mono_Set_Cursor(37, 18); Mono_Printf("%04d", CommStats_Sent_Bytes_Sec);
		Mono_Set_Cursor(37, 19); Mono_Printf("%04d", CommStats_Received_Bytes_Sec);
		Mono_Set_Cursor(45, 18); Mono_Printf("%03d", CommStats_Sent_TCP);
		Mono_Set_Cursor(45, 19); Mono_Printf("%03d", CommStats_Received_TCP);
		Mono_Set_Cursor(50, 18); Mono_Printf("%03d", CommStats_Sent_UDP);
		Mono_Set_Cursor(50, 19); Mono_Printf("%03d", CommStats_Received_UDP);
	}
}
...
int Process_Crate_Pickup(WDTCrateType type, CELL cell, FootClass *object, HousesType house, int int_arg)
{
	...
	switch (type) {
					...
					if (hptr->ActLike == actlike) {
						...
						*v166 += v200;
						...
						if (type == WDT_CRATE_RANGE) {
							...
							if (v164 == WEAPON_FLAMETHROWER || v164 == WEAPON_FLAME_TONGUE || v164 == WEAPON_CHEMSPRAY || v164 == WEAPON_STEG || v164 == WEAPON_TREX) {
								optr->Mod5 = 0;
							} else {
		...
		case WDT_CRATE_SUPER:
			...
			if ( v150 == WEAPON_FLAMETHROWER || v150 == WEAPON_FLAME_TONGUE || v150 == WEAPON_CHEMSPRAY || v150 == WEAPON_STEG || v150 == WEAPON_TREX ) {
				object->Mod5 = 0;
			} else {
			...
			} else if ( object->House == PlayerPtr ) {
				Priority_Sound_Effect(VOC_MOTOR);
				Priority_Sound_Effect(VOC_RAMBO_ROCK);
				Map.Redraw_Tab();
			}
		...
		case WDT_CRATE_BOMB:
			...
			if ( object->House == PlayerPtr ) {
				if ( anim ) {
					anim->Owner = PlayerPtr->Class->House;
				}
...
void Client_Handle_Sight(void)
{
	...
	if (Target_Legal(flag_loc)) {
		...
		if (flag_loc_cell) {
		Map.Sight_From(flag_loc_cell, 3, 0);
		}
	}
...
TechnoClass *Create_AI_Unit(void)
{
	...
	if (GameParams.IsCrates) {
		...
		if ( weap == WEAPON_FLAMETHROWER || weap == WEAPON_FLAME_TONGUE || weap == WEAPON_CHEMSPRAY || weap == WEAPON_STEG || weap == WEAPON_TREX ) {
			obj->Mod5 = 0;
		}
...
BuildingClass *Create_Building(bool dont_rand, HousesType house, HousesType gdi_or_nod)
{
	...
	case 1:
		...
		break;
	case 2:
		obj->Mod4 = WDT_Random_Pick(sole_array[2][2] / 4, sole_array[2][2]);
		break;
	case 3:
		obj->Mod5 = WDT_Random_Pick(sole_array[3][2] / 4, sole_array[3][2]);
		break;
	}
...
void Destroy_Active_AI_Objects(void)
{
	...
	for (i = 0; i < Units.Count(); i++) {
		...
		if (!hptr->IsHuman) {
			DELETE_OBJ(uptr, sizeof(AbstractClass));
			i--;
		}
	}
...
void Update_Tracking(void)
{
	...
	if (cell) {
		...
		for (int i = 0; i < sizeof(Views)/sizeof(Views[0]); i++) {
			Views[i] = Coord_Cell(Map.TacticalCoord);
		}
	}
}
...
{
	int y = 30;
	Conquer_Clip_Text_Print(Text_String(TXT_KEY_SUMMARY), 10, y, 5u, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("B", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print("Center view on your Base.", 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("D", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_DROP_ANY_FLAG), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("F", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_CENTER_ON_FLAG), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("G", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_INTO_GUARD_MODE), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_H_OR_HOME), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_CENTER_ON_UNIT), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("L", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_CLEAN_UP_PLAYER_LIST), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("N", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TOGGLE_NAMES), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("T", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TOGGLE_TRACKING), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_SHIFT_S), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TAKE_SCREENSHOT), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_ENTER), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_NORMAL_MESSAGING), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_SHIFT_ENTER), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TEAM_MESSAGING), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_ENTER_NAME_MESSAGE), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_SEND_PRIVATE_MESSAGE), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_UP_ARROW), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_REPEAT_PRV_MSG_DEST), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_ESCAPE), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_OPTIONS_MENU_DOT), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_TAB), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TOGGLE_SIDEBAR), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_1_THROUGH_0), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_PREPARE_SEND_MSG_MACRO), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("F1", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TOGGLE_HELP), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("F2", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TOGGLE_PARAM_DISPLAY), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("F3", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TOGGLE_COMMAND_DISPLAY), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_CTRL_F7_TO_F10), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_REMEMBER_VIEW), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_F7_TO_F10), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_SHOW_REMEMBERED_VIEW), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_CTRL_LEFT), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_USE_C4), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print(Text_String(TXT_ALT_LEFT), 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_BUMP_TEAMMATE), 200, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
...
void Show_Game_Parms(void)
{
	...
	y = 30;
	Conquer_Clip_Text_Print(Text_String(TXT_GAME_PARAMS_F2), 10, y, 5u, 0, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	...
	if (!GameParams.IsMaxNumAIsScaled) {
		sprintf(buffer, Text_String(TXT_HUNTERS_FROM_NUMBERS), GameParams.AIUnitsPer10min, GameParams.MaxAIUnits, GameParams.AIBuildingsPer10min, GameParams.MaxAIBuildings);
		Conquer_Clip_Text_Print(buffer, 10, y, 0xFu, 0, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
		y += 15;
	}
...
void Show_Keyboard_Commands(void)
{
	char str[300];
	int y = 30;
	Conquer_Clip_Text_Print(Text_String(TXT_ADVANCED_COMMANDS_F3), 10, y, 5u, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("*ladder", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_VIEW_LADDER), 170, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("*news", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_DISPLAY_NEWS), 170, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
	Conquer_Clip_Text_Print("*verbose", 10, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	Conquer_Clip_Text_Print(Text_String(TXT_TOGGLE_VERBOSE_MSG), 170, y, 0xFu, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 15;
...
void Thanks_to_Testers(void)
{
	...
	240, y, YELLOW, TBLACK, TPF_CENTER|TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 25;
	...
	240, y, YELLOW, TBLACK, TPF_CENTER|TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 30;
	Conquer_Clip_Text_Print("Sole Survivor Team",
	240, y, WHITE, TBLACK, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 600, 0);
	y += 25;
...
void Core_Team_Credits(void)
{
	...
	WHITE, 	TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 0);
	Messages.Add_Message("        Programmers: Tom Spencer-Smith, Neal Kettler, Dave Aldridge",
	32, 	TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 0);
	Messages.Add_Message("                           Producer: Ken Murphy",
	32, 	TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 0);
	Messages.Add_Message("                         VP/R&D: Steve Wetherill",
	32, 	TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 0);
...
extern char ButtonSixURL[980];

void Host_Send_Scenario_Change_Packet(void); // QSERVER.CPP
const char *Local_Time_As_String();
void Client_Log_Player_Message(char *); // QCLIENT.CPP
void Client_Send_Command(int, int); // QCLIENT.CPP
...
bool Handle_Typed_Message(char *message)
{
	...
	if (stricmp(message, "Get me outta here!") == 0) {
		...
		if (!GameParams.IsSquadChannel) {
			Messages.Add_Message("This isn't a squad channel... save your breath!", 15, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 0);
			Map.Flag_To_Redraw(true);
		...
		if (60 * GameParams.TimeLimit - WDTGameTimer.Time() / 60 < 120 ) {
			Messages.Add_Message("Please let this game finish first.", 15, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 0);
			Map.Flag_To_Redraw(true);
			Sound_Effect(VOC_SCOLD, VOL_FULL);
			return true;
		}
		...
		if (string) {
			...
			Messages.Add_Message("You did not give a valid password!", 15, TPF_6POINT|TPF_NOSHADOW|TPF_BRIGHT_COLOR, 0);
			Map.Flag_To_Redraw(true);
			Sound_Effect(VOC_SCOLD, VOL_FULL);
		}
