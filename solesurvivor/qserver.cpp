...
// These are only used in this file so should probably be static
...
// Fromt init.cpp
extern MainMenuSelection MenuSelection;
...
void Host_Queue_AI()
{
    ...
    if (ActivePlayers.Count() == 0) {
        if (PlayerCount > 2 && time(0) - IdleTime > 900 || time(0) - IdleTime > 7200) {
            ...
            sprintf(buff,
                    "Killing server after %d minutes idleness at time %s.\n",
                    (int)((time(0) - IdleTime) / 60),
    ...
    for (index = 0; index < ActivePlayers.Count(); index++) {
        if (ActivePlayers[index]->HousePtr->IsDefeated && ActivePlayers[index]->HousePtr->Timer1.Expired()
...
void Host_Remove_Player(int player_idx)
{
	...
	*/
    if (hptr != NULL && hptr->Class != NULL && hptr->Class->House != HOUSE_SPECTATOR
...
bool Host_Send_Houses_Update_Packet(int index)
{
    ...
    for (i = HOUSE_FIRST_TEAM; i < HOUSE_COUNT; i++) {
        ...
        if (count >= max) {
            ...
            size = (PacketLength[PACKET_HOUSE_UPDATE] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                   + sizeof(*data) * count;
            ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
            ReliableComms[index]->Send();
            SentBytesSec += size;
    ...
    for (i = 0; i < MAP_CELL_TOTAL; i++) {
        ...
        if (Map.In_Radar(i) && Overlay_Is_Crate(cptr->Overlay)) {
            ...
            if (count >= max) {
                ...
                size = (PacketLength[PACKET_CRATE] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                       + sizeof(*data) * count;
                ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                ReliableComms[index]->Send();
                SentBytesSec += size;
        ...
        for (i = 0; i < HealthPacketDatas.Count(); i++) {
            if (As_Object(HealthPacketDatas[i]->Whom) && As_Object(HealthPacketDatas[i]->Whom)->IsActive) {
                ...
                if (count >= max) {
					...
					*/
                    size = (PacketLength[PACKET_HEALTH] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                           + sizeof(*data) * count;
                    ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                    ReliableComms[index]->Send();
                    SentBytesSec += size;
...
bool Host_Send_Damage_Packet(int index)
{
    ...
    for (i = 0; i < DamagePacketDatas.Count(); i++) {
        ...
        if (count >= max) {
			...
			** Calculate actual size of packet to send.
			*/
            size = (PacketLength[PACKET_DAMAGE] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                   + sizeof(*data) * count;
            ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
            ReliableComms[index]->Send();
            SentBytesSec += size;
    ...
    for (i = 0; i < SquishPacketDatas.Count(); i++) {
        if (As_Object(SquishPacketDatas[i]->field_0) && !As_Object(SquishPacketDatas[i]->field_0)->IsActive) {
            ...
            if (count >= max) {
				...
				** Calculate actual size of packet to send.
				*/
                size = (PacketLength[PACKET_SQUISH] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                       + sizeof(*data) * count;
                ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                ReliableComms[index]->Send();
                SentBytesSec += size;
    ...
    for (i = 0; i < CapturePacketDatas.Count(); i++) {
        if (As_Object(CapturePacketDatas[i]->Whom) && As_Object(CapturePacketDatas[i]->Whom)->IsActive) {
            ...
            if (count >= max) {
				...
				** Calculate actual size of packet to send.
				*/
                size = (PacketLength[PACKET_CAPTURE] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                       + sizeof(*data) * count;
                ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                ReliableComms[index]->Send();
                SentBytesSec += size;
...
bool Host_Send_Cargo_Packet(int index)
{
    ...
    for (i = 0; i < CargoPacketDatas.Count(); i++) {
        if (As_Object(CargoPacketDatas[i]->Whom) && As_Object(CargoPacketDatas[i]->Whom)->IsActive
            && As_Object(CargoPacketDatas[i]->Cargo) && As_Object(CargoPacketDatas[i]->Cargo)->IsActive) {
            ...
            if (count >= max) {
				...
				** Calculate actual size of packet to send.
				*/
                size = (PacketLength[PACKET_CARGO] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                       + sizeof(*data) * count;
                ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                ReliableComms[index]->Send();
                SentBytesSec += size;
...
bool Host_Send_Flag_Packet(int index)
{
    ...
    for (i = 0; i < FlagPacketDatas.Count(); i++) {
        ...
        if (count >= max) {
			...
			** Calculate actual size of packet to send.
			*/
            size = (PacketLength[PACKET_FLAG] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                   + sizeof(*data) * count;
            ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
            ReliableComms[index]->Send();
            SentBytesSec += size;
...
bool Host_Send_CTF_Packet(int index)
{
    ...
    for (i = 0; i < CTFPacketDatas.Count(); i++) {
        ...
        if (count >= max) {
			...
			** Calculate actual size of packet to send.
			*/
            size = (PacketLength[PACKET_CTF] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                   + sizeof(*data) * count;
            ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
            ReliableComms[index]->Send();
            SentBytesSec += size;
            ...
            if (As_Object(MovePacketDatas[i]->Whom) && As_Object(MovePacketDatas[i]->Whom)->IsActive) {
                ...
                *data = *MovePacketDatas[i];
                ...
                if (count >= max) {
					...
					** Calculate actual size of packet to send.
					*/
                    size = (PacketLength[PACKET_MOVE] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                           + sizeof(*data) * count;
                    ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                    ReliableComms[index]->Send();
                    SentBytesSec += size;
...
void Host_Send_Target_Packet(int index)
{
    ...
    if (ReliableProtocols[index]->Queue->Num_Send() <= 2 * ReliableProtocols[index]->Queue->Max_Send() / 3) {
        for (i = 0; i < TargetPacketDatas.Count(); i++) {
            if (!As_Object(TargetPacketDatas[i]->Whom) || !As_Object(TargetPacketDatas[i]->Whom)->IsActive
                || (As_Object(TargetPacketDatas[i]->Target) && !As_Object(TargetPacketDatas[i]->Target)->IsActive)) {
                continue;
            }
            ...
            *data = *TargetPacketDatas[i];
            ...
            if (count >= max) {
				...
				** Calculate actual size of packet to send.
				*/
                size = (PacketLength[PACKET_TARGET] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                       + sizeof(*data) * count;
                ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                ReliableComms[index]->Send();
                SentBytesSec += size;
...
void Host_Send_Fire_At_Packet(int index)
{
    ...
    if (ReliableProtocols[index]->Queue->Num_Send() <= 2 * ReliableProtocols[index]->Queue->Max_Send() / 3) {
        for (i = 0; i < FireAtPacketDatas.Count(); i++) {
            if (!As_Object(FireAtPacketDatas[i]->Whom) || !As_Object(FireAtPacketDatas[i]->Whom)->IsActive
                || FireAtPacketDatas[i]->Target == 0
                || (As_Object(FireAtPacketDatas[i]->Target) && !As_Object(FireAtPacketDatas[i]->Target)->IsActive)) {
                continue;
            }
            ...
            *data = *FireAtPacketDatas[i];
            ...
            if (count >= max) {
				...
				** Calculate actual size of packet to send.
				*/
                size = (PacketLength[PACKET_FIRE_AT] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                       + sizeof(*data) * count;
                ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                ReliableComms[index]->Send();
                SentBytesSec += size;
            ...
            if (As_Object(DoTurnPacketDatas[i]->Whom) && As_Object(DoTurnPacketDatas[i]->Whom)->IsActive) {
                ...
                *data = *DoTurnPacketDatas[i];
                ...
                if (count >= max) {
					...
					** Calculate actual size of packet to send.
					*/
                    size = (PacketLength[PACKET_DO_TURN] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                           + sizeof(*data) * count;
                    ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
                    ReliableComms[index]->Send();
                    SentBytesSec += size;
        ...
        *data = *PerCellPacketDatas[i];
        ...
        if (data->Number == (char)WDT_CRATE_TELEPORT) {
            if (As_Object(data->Whom) && As_Object(data->Whom)->IsActive) {
                Process_Crate_Pickup((WDTCrateType)data->Number,
                                     data->Cell,
                                     (FootClass*)As_Object(data->Whom),
                                     data->Owner,
                                     data->_IntNumber);
            }
        }
        ...
        if (count >= max) {
            ...
            size = (PacketLength[PACKET_PCP] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                   + sizeof(*data) * count;
            ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
            ReliableComms[index]->Send();
            SentBytesSec += size;
    ...
    for (i = 0; i < TechnoPacketDatas.Count(); i++) {
        ...
        *data = *TechnoPacketDatas[i];
        ...
        if (count >= max) {
			...
			** Calculate actual size of packet to send.
			*/
            size = (PacketLength[PACKET_TECHNO] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                   + sizeof(*data) * count;
            ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
            ReliableComms[index]->Send();
            SentBytesSec += size;
	...
	*/
    ...
    for (i = 0; i < NewDeletePacketDatas.Count(); i++) {
        ...
        if (count >= max) {
			...
			** Calculate actual size of packet to send.
			*/
            size = (PacketLength[PACKET_NEW_DELETE_OBJ] - (sizeof(*data) * (MAX_PACKET_DATA_SIZE / sizeof(*data))))
                   + sizeof(*data) * count;
            ReliableProtocols[index]->Queue->Queue_Send(&packet, size);
            ReliableComms[index]->Send();
            SentBytesSec += size;
...
void Host_Send_Scenario_Change_Packet(void)
{
    ...
    packet.Scenario.Scenario = Scen.Scenario;
		...
		*/
        for (index = 0; index < ActivePlayers.Count(); index++) {
            ...
            if (ActivePlayers[index]->House == HOUSE_GOOD) {
                ActivePlayers[index]->HousePtr->Init_Data(REMAP_GOLD, HOUSE_GOOD, 0);
                ActivePlayers[index]->HousePtr->Int4 = -1;
            } else if (ActivePlayers[index]->House == HOUSE_BAD) {
                ActivePlayers[index]->HousePtr->Init_Data(REMAP_GOLD, HOUSE_BAD, 0);
                ActivePlayers[index]->HousePtr->Int4 = -1;
            } else if (ActivePlayers[index]->House == HOUSE_BLUE_TEAM) {
                ActivePlayers[index]->HousePtr->Init_Data(REMAP_LTBLUE, HOUSE_BLUE_TEAM, 0);
                ActivePlayers[index]->HousePtr->Int4 = 1;
...
void Host_Process_Remote_Admin_Packet(int index)
{
    ...
    case 4:
        ...
        for (i = 0; i < MPlayerFilenum.Count(); i++) {
            if (Scen.Scenario == MPlayerFilenum[i]) {
...
void Host_Process_Message_Packet(WDTPacketStruct* packet)
{
    ...
    if (packet->Message.To == HOUSE_NONE) {
        for (i = 0; i < ActivePlayers.Count(); i++) {
            if (ActivePlayers[i]->HousePtr->Class->House != packet->Message.From) {
                if (packet->Message.Team != -1 && packet->Message.Team != ActivePlayers[i]->HousePtr->Int4
    ...
    if (hptr->Class->House == HOUSE_ADMIN && packet->Message.To == HOUSE_NONE) {
        ...
        Messages.Add_Message(
            buffer, MPlayerTColors[packet->Message.Team + 1], TPF_6POINT | TPF_NOSHADOW | TPF_BRIGHT_COLOR, 0);
        Map.Flag_To_Redraw(false);
    } else {
        if (packet->Message.To == HOUSE_NONE) {
            sprintf(buffer, "%s: %s", hptr->Name, packet->Message.MessageBuffer);
            ...
            sprintf(buffer,
                    "%s (to %s): %s",
                    hptr->Name,
                    HouseClass::As_Pointer(packet->Message.To)->Name,
...
void Host_Process_Command_Packet(WDTPacketStruct* packet, int index)
{
    ...
    if (packet->Command.State == 0) {
        ...
        do {
            ...
            ini.Get_String("Multiplayer", name, "", string, sizeof(string));
...
bool Host_Swollow_Packets_While_Waiting(ReliableProtocolClass* protocol)
{
    ...
    for (index = 0; index < protocol->Queue->Num_Send(); index++) {
        ...
        if (send) {
            ...
            if (head->Type == PACKET_HEALTH || head->Type == PACKET_MOVE || head->Type == PACKET_TARGET
                || head->Type == PACKET_FIRE_AT || head->Type == PACKET_DO_TURN || head->Type == PACKET_FRAMERATE) {
